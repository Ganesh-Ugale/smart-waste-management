...
def create_bin():
    try:
        ...
        required_fields = ['location', 'capacity', 'address', 'binName']
        ...
        if not bin_id:
            return jsonify({"success": False, "error": "Failed to create bin in database"}), 500
...
def optimize_route():
    try:
        ...
        bins_data = firebase_service.realtime_db.child("bins").get()
        if not bins_data:
            return jsonify({"success": False, "message": "No bins found in the database."})
        ...
        for bin_id, bin_info in bins_data.items():
            ...
            if bin_info.get("currentFillLevel", 0) >= 70:
                bins_list.append({
                    ...
                    "binName": bin_info.get("binName", f"Bin-{bin_id[-4:]}")
                })
...
def truck_full_route():
    try:
        ...
        # 🔥 Get remaining bins
        bins_data = firebase_service.realtime_db.child("bins").get() or {}
