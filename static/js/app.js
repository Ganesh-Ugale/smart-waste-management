...
async function generateRoute() {
    ...
    try {
        ...
        if (!checkResult.success) {
            alert(checkResult.message || "Not enough bins need collection (minimum 3 bins at ≥70%).");
            return;
        }
        ...
        if (!response.ok) {
            alert("Server error while generating route. Please try again.");
            return;
        }
        const result = await response.json();
        if (!result.success) {
            alert(result.message || "Could not generate route.");
            return;
        }
        ...
        console.error(error);
        alert("Server error while optimizing route. Please try again.");
    }
}
...
async function changeRoute() {
    ...
    try {
        ...
        if (!response.ok) {
            alert("Server error while re-optimizing route. Please try again.");
            return;
        }
        const result = await response.json();
        if (!result.success) {
            alert(result.message || "Could not re-optimize route.");
            return;
        }
...
async function truckFullRoute() {
    ...
    try {
        ...
        if (!response.ok) {
            alert("Server error in truck route. Please try again.");
            return;
        }
        const result = await response.json();
        if (!result.success) {
            alert(result.message || "Could not generate truck route.");
            return;
        }
...
function setupEventListeners() {
    ...
    if (addForm) {
        addForm.addEventListener('submit', async function (e) {
            ...
            try {
                const addRes = await fetch(`${API_BASE_URL}/bins`, {
                    method: 'POST',
                ...
                if (!addRes.ok) {
                    alert("Server error while adding bin. Please try again.");
                    return;
                }
                ...
                if (addResult.success) {
                    ...
                    alert("Failed to add bin: " + (addResult.error || "Unknown error"));
                }
            } catch (err) {
